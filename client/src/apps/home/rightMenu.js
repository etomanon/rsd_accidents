...
class RightMenu extends Component {
      ...
      },
      {
        ...
        comp: () => (
          <Menu
            ...
            critical={this.props.map.critical}
            criticalToggle={this.props.criticalToggle}
          />
        )
      }
    ];
    ...
    return (
      <Tooltip key={name} title={title} placement="left">
        <div
          ...
          className={`mt-10 button button--menu ${this.state[name] &&
            "button--active"}`}
        >
          <i className={icon} />
        </div>
  ...
  submenu = (name, comp) => {
    return (
      <div
        key={name}
        className={`right-menu__submenu ${this.state[name] &&
          "right-menu__submenu--active"}`}
      >
        {comp()}
      </div>
    );
  ...
  render() {
    return (
      <div
        className={`right-menu ${valuesIn(this.state).some(v => v) &&
          "right-menu--active"}`}
      >
        <div className="right-menu__options container-column">
          {this.items.map(item =>
            this.button(item.name, item.title, item.icon)
          )}
        </div>
